 ...
 
{
	global $post;
	//echo $post->post_type.'====';
	if( 'portfolio.php' == get_post_meta($post->ID, '_wp_page_template', true) || 'bl_portfolio' == $post->post_type ){
	?>
...
remove_action( 'init', 'wpt_slider_posttype' );
...
function remove_unwanted_widgets(){
	unregister_sidebar('portfolio-sidebar');
	unregister_sidebar('gallery');
}
