...
<div id="portfolio-gallery" class="internal_page_items internal_page_gallery">
    ...
    while( $gallery->have_posts() ) : $gallery->the_post(); ?>
        ...
        <li data-id="id-<?php echo $i; ?>" class="<?php if( ($i % $postsPerRow)==1 ): ?>group<?php endif ?> <?php if(!empty($terms)) foreach ($terms as $term) { echo strtolower(preg_replace('/\s+/', '-', $term->slug)) . ' '; }  ?>">
        
            <div class="internal_page_item internal_page_item_gallery <?php echo $classes ?> <?php if($isFirstInRow): ?>internal_page_item_first<?php endif ?><?php if($isLastInRow): ?>internal_page_item_last<?php endif ?>">
                <a href="<?php echo $image[0] ?>" rel="prettyPhoto[gallery]" title="<?php the_title() ?>" class="thumb"><?php the_post_thumbnail( 'thumb_home_page', array( 'class' => 'picture' ) ) ?></a>
