...
// 	add_image_size( 'portfolio-single', 1180, 535, true ); 		// portfolio - single
	...
	add_image_size( 'blog', 			1180, 502, true ); 		// blog
	add_image_size( 'blog-widget', 		173,  120, true ); 		// blog - widget
...
function mfn_pagination( $query = false ){
	...
	$translate['next'] = mfn_opts_get('translate') ? mfn_opts_get('translate-next','Next page &rsaquo;') : __('Next page &rsaquo;','falco');
	$translate['prev'] = mfn_opts_get('translate') ? mfn_opts_get('translate-prev','&lsaquo; Prev page') : __('&lsaquo; Prev page','falco');
...
class New_Walker_Category extends Walker_Category {
	function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $use_desc_for_title == 0 || empty($category->description) )
			$link .= 'title="' . esc_attr( sprintf(__('View all posts filed under %s','falco'), $cat_name) ) . '"';
		else
...
function mfn_breadcrumbs() {
	...
	$translate['home'] = mfn_opts_get('translate') ? mfn_opts_get('translate-home','Home') : __('Home','falco');
	$translate['you-are-here'] = mfn_opts_get('translate') ? mfn_opts_get('translate-you-are-here','You are here:') : __('You are here:','falco');

	echo '<ul class="breadcrumbs">';
// 	echo '<li>'. $translate['you-are-here'] .'</li>';
	echo '<li class="home"><i class="icon-home"></i> <a href="'. $homeLink .'">'. $translate['home'] .'</a> <span>/</span></li>';

	// Blog Category
	if ( is_category() ) {
		echo '<li><a href="'. curPageURL() .'">' . __('Archive by category','falco').' "' . single_cat_title('', false) . '"</a></li>';
...
function mfn_post_type( $postID, $icon = false ){
	...
	if( get_post_meta( $postID, 'mfn-post-slider', true ) ){
		$post_type = 'slider';
		$post_icon = 'icon-resize-horizontal';
 ...
 * Post Thumbnails
 * --------------------------------------------------------------------------- */
function mfn_post_thumbnail( $postID, $zoom = false ){
...
function mfn_portfolio_details( $postID ){
	...
	$translate['category'] 			= mfn_opts_get('translate') ? mfn_opts_get('translate-category','Category:') : __('Category:','falco');
	$translate['client'] 			= mfn_opts_get('translate') ? mfn_opts_get('translate-client','Client:') : __('Client:','falco');
	$translate['date'] 				= mfn_opts_get('translate') ? mfn_opts_get('translate-date','Date:') : __('Date:','falco');
	$translate['task'] 				= mfn_opts_get('translate') ? mfn_opts_get('translate-task','Task:') : __('Task:','falco');
	...
	<ul class="project_details">
		...
		<?php if( $task = get_post_meta(get_the_ID(), 'mfn-post-task', true) ): ?>
			<li class="task">
				...
				<p><i class="icon-tasks"></i>&nbsp;<?php echo $task; ?></p>
			</li>
		<?php endif; ?>
		...
		<?php if( $categories && get_post_meta(get_the_ID(), 'mfn-post-categories', true) ): ?>
			<li class="categories">
				...
				<p><i class="icon-folder-close-alt"></i>&nbsp;<?php echo $categories; ?></p>
			</li>
		<?php endif; ?>
		...
		<?php if( $link = get_post_meta(get_the_ID(), 'mfn-post-link', true) ): ?>
			<li class="url">
				...
				<p><i class="icon-external-link"></i>&nbsp;<a href="<?php echo $link; ?>" target="_blank"><?php echo $translate['visit-online']; ?></a></p>
			</li>
		<?php endif; ?>
...
function mfn_comments_number() {
	$translate['comment'] = mfn_opts_get('translate') ? mfn_opts_get('translate-comment','comment') : __('comment','falco');
	$translate['comments'] = mfn_opts_get('translate') ? mfn_opts_get('translate-comments','comments') : __('comments','falco');
	$translate['comments-off'] = mfn_opts_get('translate') ? mfn_opts_get('translate-comments-off','comments off') : __('comments off','falco');
...
function mfn_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'source'   				=> LIBS_DIR .'/plugins/contact-form-7.zip', // The plugin source
			'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
		),
		array(
			'name'     				=> 'Recent Tweets', // The plugin name
			'slug'     				=> 'recent-tweets-widget', // The plugin slug (typically the folder name)
			'source'   				=> LIBS_DIR .'/plugins/recent-tweets-widget.zip', // The plugin source
	...
	$config = array(
		'domain'       		=> 'falco',         			// Text domain - likely want to be the same as your theme.
		...
		'strings'      		=> array(
			'page_title'                       			=> __( 'Install Required Plugins', 'falco' ),
			'menu_title'                       			=> __( 'Install Plugins', 'falco' ),
			'installing'                       			=> __( 'Installing Plugin: %s', 'falco' ), // %1$s = plugin name
			'oops'                             			=> __( 'Something went wrong with the plugin API.', 'falco' ),
			...
			'return'                           			=> __( 'Return to Required Plugins Installer', 'falco' ),
			'plugin_activated'                 			=> __( 'Plugin activated successfully.', 'falco' ),
			'complete' 									=> __( 'All plugins installed and activated successfully. %s', 'falco' ), // %1$s = dashboard link
