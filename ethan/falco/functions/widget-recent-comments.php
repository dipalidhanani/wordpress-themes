					...
					foreach($comments as $comment)
					{
						...
						$output .= '<li>';
							$output .= '<div class="text">';
								$output .= '<p>"'. mfn_get_comment_excerpt($comment->comment_ID, 14) .'"</p>';
								$output .= '<div class="arrow"></div>';
							$output .= '</div>';
							$output .= '<p class="author">'.strip_tags($comment->comment_author) .', <a href="'. $url .'">'. get_the_title($comment->comment_post_ID) .'</a></p>';
						$output .= '</li>';
					}
				$output .= '</ul>';
