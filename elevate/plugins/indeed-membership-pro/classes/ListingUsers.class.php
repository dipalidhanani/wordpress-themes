					...
					if (is_array($filter_value)){
						if ($temp_field_type=='number'){
							...
							if (isset($filter_value[0]) && isset($filter_value[1])){
								...
								$q .= " AND ( CAST($alias.meta_value as INTEGER))>(CAST('{$filter_value[0]}' as INTEGER)) ";
							} else if (isset($filter_value[1])){
								$q .= " AND (CAST($alias.meta_value as INTEGER))<(CAST('{$filter_value[1]}' as INTEGER)) ";
							}
						...
						} else if ($temp_field_type=='checkbox' || $temp_field_type=='multiselect'){
			 ...
			 if ($field_slug){
				 ...
				 switch ($type){
					...
					case 'checkbox':
						...
						break;						
					case 'number':
						$q = "SELECT CAST(meta_value AS INTEGER) as min FROM $table WHERE meta_key='$field_slug' AND meta_value!='' ";
						if ($ids_in){
							$q .= " AND user_id IN ($ids_in) ";
						}
						$q .= " ORDER BY CAST(meta_value AS INTEGER) ASC LIMIT 1;";
						$data = $wpdb->get_row($q);
						if (isset($data->min)){
							$array['min'] = $data->min;
						}
						$q = "SELECT CAST(meta_value AS INTEGER) as max FROM $table WHERE meta_key='$field_slug' AND meta_value!='' ";
						if ($ids_in){
							$q .= " AND user_id IN ($ids_in) ";
						}						
						$q .= " ORDER BY CAST(meta_value AS INTEGER) DESC LIMIT 1;";
						$data = $wpdb->get_row($q);	
						if (isset($data->max)){
							$array['max'] = $data->max;
						}					
