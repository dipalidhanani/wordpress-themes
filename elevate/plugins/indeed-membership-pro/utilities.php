...
function ihc_set_level_trial_time_for_no_pay($l_id=-1, $u_id=0){
	/*
	 * SET THE TRIAL TIME
	 * @param int, int
	 * @return none
...
function ihc_handle_levels_assign($uid=0, $lid=-1){
	/*
...
function ihc_do_user_approve($uid=0){
	...
	if ($uid){
		$data = get_userdata($uid);
		if ($data && isset($data->roles) && isset($data->roles[0]) && $data->roles[0]=='pending_user'){
			$user_id = wp_update_user(array( 'ID' => $uid, 'role' => 'subscriber'));		
			if ($user_id==$uid){
