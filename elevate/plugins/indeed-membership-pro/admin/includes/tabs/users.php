				...
				<div class="span3">
					<div class="iump-form-line iump-no-border">
						<select name="orderby_user">
							...
							<option value="login_name" <?php echo (isset($_REQUEST['orderby_user']) && $_REQUEST['orderby_user']=='login_name') ? 'selected' : ''; ?>><?php _e('Username', 'ihc');?></option>
		...
		<form method="post" action="" style="margin-top:20px;">
			...
			<div>
				<?php 					
					...
					$current_time = time();
					if ($search_term != ''){
						if (!function_exists('ihc_pre_user_query')){
					...
					$url  = admin_url('admin.php?page=ihc_manage&tab=users&ihc_limit=' . $limit);
							  		...
							  		foreach ($users as $user){								  			
							  			...
							  			?>
			    						   		<tr id="<?php echo "ihc_user_id_" . $user->data->ID;?>" class="<?php if($i%2==0) echo 'alternate';?>" onMouseOver="ihc_dh_selector('#user_tr_<?php echo $user->data->ID;?>', 1);" onMouseOut="ihc_dh_selector('#user_tr_<?php echo $user->data->ID;?>', 0);">
			    						   			...
			    						   			<td>
														...
														<?php 
															if ($is_uap_active && !empty($indeed_db)){		
																$is_affiliate = $indeed_db->is_user_affiliate_by_uid($user->data->ID);
																if ($is_affiliate){
																	?>
														...
														<div style="visibility: hidden;" id="user_tr_<?php echo $user->data->ID;?>">
															...
															| 
															<a onClick="ihc_delete_user_prompot(<?php echo $user->data->ID;?>);" href="javascript:return false;" style="color: red;"><?php _e('Delete', 'ihc');?></a>
															<?php 
			    						   						...
			    						   						if (strpos($user_levels, ',')!==FALSE){
			    						   							...
			    						   							foreach ($levels as $level_id){
			    						   								...
			    						   								if ($temp_data && isset($temp_data['name'])){			    						   									
					    						   							...
					    						   							$is_expired_class = '';
					    						   							$level_title = "Active";
																			if (isset($time_arr['expire_time']) && $current_time>strtotime( $time_arr['expire_time'] ) ){			    						   								
					    						   								$is_expired_class = 'ihc-expired-level';
																				$level_title = "Hold/Expired";
					    						   							}
			    						   									?>
			    						   									<div class="level-type-list <?php echo $is_expired_class;?>" title="<?php echo $level_title?>"><?php echo $temp_data['name'];?></div>
			    						   									<?php 	
																		}
			    						   							}
			    						   						} else {
			    						   							...
			    						   							if ($temp_data){
			    						   								...
			    						   								echo '<div class="level-type-list ' . $is_expired_class . '" title="'.$level_title.'">'.$temp_data['name'].'</div>';	    						   								
			    						   							}
			    						   						}	
			    						   					}
			    						   				?>
													...
													<td>
														<?php 
														$ord_count = ihc_get_user_orders_count($user->data->ID);
														if(isset($ord_count) && $ord_count > 0): ?>
														<div class="ihc_frw_button"> <a href="<?php echo admin_url('admin.php?page=ihc_manage&tab=orders&uid=') . $user->data->ID;?>" target="_blank">Orders</a></div>
														<?php endif;?>
														<?php unset($ord_count);?>
														<?php if (ihc_is_magic_feat_active('user_reports') && Ihc_User_Logs::get_count_logs('user_logs', $user->data->ID)):?>
															<div class="level-type-list"> <a href="<?php echo admin_url('admin.php?page=ihc_manage&tab=view_user_logs&type=user_logs&uid=') . $user->data->ID;?>" target="_blank" style="color: #fff;"><?php _e('User Reports', 'ihc');?></a></div>
														<?php endif;?>															
