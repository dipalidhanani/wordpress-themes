...
//echo "ddd";
		    	...
		    	$complete_html .= '<article class="' . join( ' ', get_post_class( array( 'course', 'post' ), $course_item->ID ) ) . '">';
		    		...
		    		$complete_html .= '<section class="entry">';
		    			...
		    			$complete_html .= '<p class="sensei-course-meta">';
		    		    	...
		    		    	// Lesson count for this author
		    		    	$complete_html .= '<span class="course-lesson-count">' . $woothemes_sensei->post_types->course->course_lesson_count( absint( $course_item->ID ) ) . ' Modules</span>';
		    		    	// Course Categories
		    		    	if ( '' != $category_output ) {
						...
						if( $manage ) {
							...
							if ( has_filter( 'sensei_results_links' ) || $has_quizzes ) {
								...
								if(get_the_author_meta( 'group_individual_account', get_current_user_id() ) == 'Group Account'){
									$complete_html .= '<a class="button sensei-certificate-link" href="/add-certificate-users">' . apply_filters( 'sensei_view_results_text', __( 'Add Certificate Users', 'woothemes-sensei' ) ) . '</a>';
								}else{
		...
		if( $manage ) {
			if($no_complete_message == ''){
				$no_active_message = apply_filters( 'sensei_no_active_courses_user_text', __( 'You have already completed ELEVATE, click on the Completed Course tab to review the program.', 'woothemes-sensei' ) );
			}else{
			$no_active_message = apply_filters( 'sensei_no_active_courses_user_text', __( 'You have not yet been approved for the Elevate course material. Please <a href="/support">contact</a> support with any questions comments or concerns.', 'woothemes-sensei' ) );
			}
...
//echo "lll".$manage;
		...
		if(($userexpirydate != '') && (date('Y-m-d') > $userexpirydate)){
            ...
            <a href="/register-2?renew=1" class="continue_link">Renew</a>
            </div>
            <?php 
			}else{
		?>
...
<?php global $current_user;
	...
	?>
     <div style="font-weight:bold; color: rgb(31, 123, 31);">Your account renewed successfully.. Your next expiry date is <?php echo get_the_author_meta( 'account_expiry_date', $current_user->ID ); ?></div>
	<?php } ?>	
            <ul>
		    	<?php if($current_user_role != 'corporate_user'){ ?><li><a href="#active-courses"><?php echo apply_filters( 'sensei_active_courses_text', __( 'Active Courses', 'woothemes-sensei' ) ); ?></a></li> <?php } ?>
		    	 <?php if($current_user_role == 'administrator' || 1==1){ ?><li><a href="#completed-courses"><?php echo apply_filters( 'sensei_completed_courses_text', __( 'Completed Courses', 'woothemes-sensei' ) ); ?></a></li><?php } ?>
                 <?php if(($current_user_role == 'administrator') || ($current_user_role == 'corporate_user')){ ?><li><a href="#my-learners"><?php echo apply_filters( 'sensei_completed_courses_text', __( 'My Learners', 'woothemes-sensei' ) ); ?></a></li><?php } ?>
		    </ul>
<?php if($current_user_role != 'corporate_user'){ ?>
            ...
            <?php } ?>
<?php if($current_user_role == 'administrator' || 1==1){ ?>
       ...
       <?php if(($current_user_role == 'administrator') || ($current_user_role == 'corporate_user')){ ?>     
            <div id="my-learners">
            <?php if($current_user_role == 'corporate_user'){ ?>
            <p style="clear:both"><a href="/corporate-sub-users-registration" class="continue_link">Add User</a></p>
            <?php } ?>
            <h1>My Learners:</h1>
            <p>
            <?php 
	...
	function widget( $args, $instance ) {
	 ...
	 $active_count = $completed_count = 0;
		...
		$next_lesson_id1 = absint( $nav_id_array1['next_lesson'] );
				...
				<li class="course-progress-lesson <?php echo $lid."~".$lesson->ID; ?> <?php echo end($lesson_array)->ID; ?> rrr5 <?php echo $classes; ?>">
...
function wpse_19692_registration_redirect() {
    return '<a href="'.esc_url( home_url( "/register-2" ) ).'">Register</a>';
}
...
die();  */
  wp_redirect ('/add-certificate-users-success');
  die();
		...
		if($_GET['renew'] == '1'){
			...
			global $current_user;
			 ...
			 $userid = $current_user->ID;
				...
				$billing_payment_method = esc_attr($_POST['billing_payment_method']);
				...
				update_user_meta ($userid, 'billing_payment_method', $billing_payment_method);
