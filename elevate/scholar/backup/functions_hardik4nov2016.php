	...
	<?php } ?>	
            <ul>
		    	<?php if($current_user_role != 'corporate_user'){ ?><li><a href="#active-courses"><?php echo apply_filters( 'sensei_active_courses_text', __( 'Active Courses', 'woothemes-sensei' ) ); ?></a></li> <?php } ?>
		    	 <?php if($current_user_role == 'administrator' || $current_user_role != 'corporate_user' || $current_user_role == 'individual_user'){ ?><li><a href="#completed-courses"><?php echo apply_filters( 'sensei_completed_courses_text', __( 'Completed Courses', 'woothemes-sensei' ) ); ?></a></li><?php } ?>
...
if ( ! wp_next_scheduled( 'my_task_hook' ) ) {
 ...
 wp_schedule_event(strtotime(date('Y-m-d 00:01:00')), 'daily', 'updateUserScores');
}
...
function my_task_function() {
	...
	// wp_mail( 'dipali@2webdesign.com', 'Test email', 'Test Message email.');
...
//foreach ( $blogusers as $user ) {
   ...
   if($datediff == 30){
	...
	if($account_expiry_date <= date('Y-m-d')){		
		update_user_meta ($userid, 'account_status', 'deactivate');
