...
// Output HTML
if ( ( 0 < $previous_lesson_id ) || ( 0 < $next_lesson_id ) ) { ?>
  <div class="inner-left11">
	<nav id="post-entries" class="post-entries 222 fix" style="padding-top:0px;">
        <?php if ( 0 < $previous_lesson_id ) { ?>
            <a href="<?php echo esc_url( get_permalink( $a[0] ) ); ?>" rel="prev" class="prev-button">Back</a>
        <?php } ?>
        <?php if( !empty($comments) ) { ?>
        <?php if ( 0 < $next_lesson_id ) { ?>
            <a href="<?php echo esc_url( get_permalink( $next_lesson_id ) ); ?>" rel="prev" class="prev-button quiz-next-btn">
             Done</a>             
          <?php } ?> <?php } //else{ ?>
      <!--  <div class="nav-next fr"> <a href="?run=first">Mark Lesson Complete</a></div>-->
         <?php //} ?>
