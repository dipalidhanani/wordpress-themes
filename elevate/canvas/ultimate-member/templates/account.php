				...
				foreach( $ultimatemember->account->tabs as $k => $arr ) {
					...
					foreach( $arr as $id => $info ) { extract( $info );
						...
						if ( isset($info['custom']) || um_get_option('account_tab_'.$id ) == 1 || $id == 'general' ) {
							...
							<?php /*?><div class="um-account-nav uimob340-show uimob500-show"><a href="#" data-tab="<?php echo $id; ?>" class="<?php if ( $id == $current_tab ) echo 'current'; ?>"><?php echo $title; ?>
							...
							echo '<div class="account-tabcont um-account-tab-'.$id.'" data-tab="'.$id.'">';
