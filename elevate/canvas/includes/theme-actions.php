...
// Navigation
add_action( 'woo_header_inside','woo_nav', 11 );
...
function woo_footer_left () {
	...
	if( 'true' == $settings['footer_left'] ) {
		$html .= '<p class="copytext">' . stripslashes( $settings['footer_left_text'] ) . '</p>';
	} else {
		$html .= '[site_copyright]';
	}
...
function woo_footer_right () {
	...
	if( 'true' == $settings['footer_right'] ) {
		$html .= '<p class="designby">' . stripslashes( $settings['footer_right_text'] ) . '</p>';
	} else {
		$html .= '[site_credit]';
	}
...
<?php
	...
	if ( ( '' != $settings['logo'] ) ) {
		...
		echo '<a href="' . esc_url( $site_url ) . '" title="' . esc_attr( $site_description ) . '"><img src="' . esc_url( $logo_url ) . '" alt="' . esc_attr( $site_title ) . '" class="logo" /></a>' . "\n";
	} // End IF Statement
