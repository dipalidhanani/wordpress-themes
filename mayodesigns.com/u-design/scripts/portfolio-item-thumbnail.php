...
function get_portfolio_item_thumbnail( $num_cols='3', $width='260', $height='160', $do_not_link_adjacent_items = 'no', $echo = true ) {
    ...
    if ( function_exists('get_the_image') ) { // the case when "Get The Image" plugin is available (installed and activated)
		...
		$output .= '<a'.$rel_attr.' href="'.$preview_item.'" title="'.$preview_item_title.'"'.$portfolio_item_link_target.'><img class="hover-opacity 11" src="'.udesign_process_image( $portfolio_thumb_as_array[url], $width, $height, 1, 90 ).'" width="'.$width.'" height="'.$height.'" alt="'.$portfolio_thumb_as_array[alt].'" /></a>';
