...
function ninja_forms_export_subs_to_csv( $sub_ids = '', $return = false ){
	global $ninja_forms_fields, $ninja_forms_processing,$wpdb;
	...
	if(is_array($sub_ids) AND !empty($sub_ids)){
		...
		foreach($sub_ids as $id){
			...
			$agh_data = $wpdb->get_var( "select sub_data from {$submission_tbl} where id=".$id );
			$agh_data = unserialize( $agh_data );
			if(is_array($sub_row['data']) AND !empty($sub_row['data'])){
				...
				if( 'Elektriciteit & Gas' ==  $agh_data['product_type'] ){
					$prod_name = explode('{@}', $agh_data['product_name']);
					$contract_period = explode('{@}', $agh_data['contract_period']);
					$product_name = 'Elektriciteit: '.$prod_name[0].', Gas: '.$prod_name[1];
					$cont_period = 'Elektriciteit: '.$contract_period[0].', Gas: '.$contract_period[1];
				}else{
					$product_name = $agh_data['product_name'];
					$cont_period = $agh_data['contract_period'].' jaar';
				}
				array_push( $value_array[$x], $product_name, $agh_data['product_type'], $cont_period, $agh_data['meter_type'], $agh_data['person_value'], $agh_data['srch_electricity'], $agh_data['srch_electricity2'], $agh_data['srch_gas'], $agh_data['include_discount'] );
			}
			$x++;
			
		}
	}
	array_push( $label_array[0], 'Produktnaam', 'Type product', 'Contract periode', 'Meter Type', 'Person Value', 'Search Electricity', 'Search Electricity2', 'Search Gas', 'Include Discount' );
