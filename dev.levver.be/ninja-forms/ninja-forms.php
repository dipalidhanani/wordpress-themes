<?php
/*
Plugin Name: Ninja Forms *CUSTOM - DO NOT UPDATE!*
...
add_action('aghadi_ninja_hidden_fields', 'aghadi_ninja_hidden_fields_callback', 10, 2);
function aghadi_ninja_hidden_fields_callback( $form_id ){
	$product   = new product("zipSearch",$_GET);
	$product_name = ($_GET['electric_ID'] && $_GET['gas_ID']) ? $product->_product['electric']['elec_product_name'] .' {@} '. $product->_product['gas']['gas_product_name'] : ifNull($product->_product['electric']['elec_product_name'],$product->_product['gas']['gas_product_name']);
	$product_type = (($_GET['electric_ID'] > 0 && $_GET['gas_ID'] > 0) ? "Elektriciteit & Gas" : ($_GET['gas_ID'] > 0 ? "Gas" : "Elektriciteit"));
	$contact_period = ($_GET['electric_ID'] && $_GET['gas_ID']) ? $product->_product['electric']['elec_contract_duration_year'] . ' jaar {@} '.$product->_product['gas']['gas_contract_duration_year'] .' jaar ' : ifNull($product->_product['electric']['elec_contract_duration_year'],$product->_product['gas']['gas_contract_duration_year']);
?>
	<input type="hidden" name="product_name" value="<?php echo $product_name;?>" />
	<input type="hidden" name="product_type" value="<?php echo $product_type;?>" />
	<input type="hidden" name="contract_period" value="<?php echo $contact_period;?>" />
	<input type="hidden" name="meter_type" value="<?php echo $_REQUEST['meter_type'];?>" />
	<input type="hidden" name="person_value" value="<?php echo $_REQUEST['person_value'];?>" />
	<input type="hidden" name="srch_electricity" value="<?php echo $_REQUEST['srch_electricity'];?>" />
	<input type="hidden" name="srch_electricity2" value="<?php echo $_REQUEST['srch_electricity2'];?>" />
	<input type="hidden" name="srch_gas" value="<?php echo $_REQUEST['srch_gas'];?>" />
	<input type="hidden" name="include_discount" value="<?php echo $_REQUEST['include_discount'];?>" />
<?php 	
}

function aghadi_ninja_form_update(){
	/*global $wpdb, $current_user;
	get_currentuserinfo();
	$submission_tbl = "";
	if( defined('NINJA_FORMS_SUBS_TABLE_NAME') ){
		$submission_tbl = NINJA_FORMS_SUBS_TABLE_NAME;
	}else{
		$submission_tbl = $wpdb->prefix . "ninja_forms_subs";
	}
	$max_sub_id = $wpdb->get_var("select max(id) from {$submission_tbl} where action='submit'");
	$data = $_SESSION['sid_data'];
	$wpdb->query("update {$submission_tbl} set sub_data='{$data}' where id={$max_sub_id}");
	$_SESSION['sid_data'] = "";
	unset($_SESSION['sid_data']);*/
}
add_action('wp_footer','call_jquery_to_remove_form_element');
function call_jquery_to_remove_form_element(){
?>
	<script type="text/javascript">
		jQuery.noConflict();
		jQuery(document).ready(function(){
			jQuery('.ninja-forms-form div.ninja-forms-all-fields-wrap div').each(function(){
				<?php 
					if( @$_GET['electric_ID'] > 0 && ( @$_GET['gas_ID'] == "" || $_GET['gas_ID'] == 0 || $_GET['gas_ID'] < 0) ){ ?>
						if( jQuery(this).hasClass('cust-gas') ){
							jQuery(this).remove();
						}
						if( jQuery(this).hasClass('cust-gas-wrap') ){
							jQuery(this).remove();
						}
						
				<?php 
					}elseif( @$_GET['gas_ID'] > 0 && ( @$_GET['electric_ID'] == "" || $_GET['electric_ID'] == 0 || $_GET['electric_ID'] < 0) ){					
				?>
						if( jQuery(this).hasClass('cust-electric') ){
							jQuery(this).remove();
						}
						if( jQuery(this).hasClass('cust-electric-wrap') ){
							jQuery(this).remove();
						}
				<?php 
					}
